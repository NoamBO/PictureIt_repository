 ...
 */
package com.pictureit.noambaroz.beauticianapp.utilities.view;
...
public class MySwitch extends CompoundButton {
	...
	private Layout makeLayout(CharSequence text) {
		return new StaticLayout(text, mTextPaint, (int) android.util.FloatMath.ceil(Layout.getDesiredWidth(text,
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mOrientation == HORIZONTAL) {
			...
			if ((this.mPushStyle) || (mTextOnThumb == false)) {
				switchWidth = Math.max(maxTextWidth + mTrackTextPadding * 2 + mTrackPaddingRect.left
						+ mTrackPaddingRect.right, mThumbWidth);
			}
		}
		...
		if (mOrientation == VERTICAL) {
			switchHeight = mOnLayout.getHeight() + mOffLayout.getHeight() + mThumbTextPadding * 2 + mThPad.top
					+ mThPad.bottom + mTrackPaddingRect.bottom + mTrackPaddingRect.top + mTrackTextPadding * 2;
			if (mTextOnThumb == false) {
				switchHeight = Math.max(mThumbHeight + maxTextHeight
						+ (mTrackPaddingRect.bottom + mTrackPaddingRect.top) / 2 + mTrackTextPadding * 2,
						mSwitchMinHeight);
			}
			if (this.mPushStyle) {
				switchHeight = Math.max(mSwitchMinHeight, maxTextHeight + mThumbHeight + mTrackTextPadding
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (mPushStyle) {
			...
			final int maxTextWidth = Math.max(mOnLayout.getWidth(), mOffLayout.getWidth());
			final int maxTextHeight = Math.max(mOnLayout.getHeight(), mOffLayout.getHeight());
			int width = maxTextWidth * 2 + mTrackPaddingRect.left + mTrackPaddingRect.right + mThumbWidth
					+ mTrackTextPadding * 4;
			int height = mSwitchBottom - mSwitchTop;

			if (mOrientation == VERTICAL) {
				height = mTrackPaddingRect.top + mTrackTextPadding + maxTextHeight + mTrackTextPadding + mThumbHeight
						+ mTrackTextPadding + maxTextHeight + mTrackTextPadding + mTrackPaddingRect.bottom;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mOrientation == VERTICAL) {
			...
			if (mPushStyle) {
				...
				backingLayer.translate(0,
						maxTextHeight + mTrackTextPadding * 2 + (maxTextHeight - mOnLayout.getHeight()) / 2
								+ mThumbHeight);// + mThPad.left +
												// mThPad.right,)
				if (mDrawableOn != null)
				...
				if (mTextColors != null) {
					mTextPaint
							.setColor(mTextColors.getColorForState(getDrawableState(), mTextColors.getDefaultColor()));
				}
		...
		if (mOrientation == HORIZONTAL) {
			...
			int dxOffText = mTextOnThumb ? (thumbL + thumbR) / 2 - mOffLayout.getWidth() / 2 + mTrackTextPadding
			...
			if (mPushStyle) {
				...
				if (mDrawableOff != null)
					mDrawableOff.draw(backingLayer);
				backingLayer.restore();
				backingLayer.translate(maxTextWidth + mTrackTextPadding * 2 + (maxTextWidth - mOnLayout.getWidth()) / 2
	...
	private int getThumbScrollRange() {
		...
		if (mOrientation == HORIZONTAL)
			range = mSwitchWidth - mThumbWidth - mTrackPaddingRect.left - mTrackPaddingRect.right + mThumbExtraMovement
					* 2;
		if (this.mPushStyle)
